<?php
	...
	if(!function_exists('sendmail'))
	{
		function sendmail() {
			...
			if(get_option('woocommerce_tab_active_captcha')=='yes')
			{
				require_once('frontend-ui/recaptchalib.php');
				if ($_POST["recaptcha_response_field"]) {
					$resp = recaptcha_check_answer (get_option('woocommerce_tab_private_key'),
													$_SERVER["REMOTE_ADDR"],
					...
					if ($resp->is_valid) {
							...
							$headers = __('From: ',EXTRA_WOO_TABS_TEXTDOMAN). $to . "\r\n" .
							'Reply-To: ' . $email . "\r\n";
					...
					padding-bottom: 35px;width:25%;font-weight: bold;">
						...
						echo "<span style='color:red'><h3>".__( 'Error, Please Try Again! The Captcha is Wrong!' , EXTRA_WOO_TABS_TEXTDOMAN )."</h3></span>";
						//$error = $resp->error;
						//echo recaptcha_get_html(get_option('woocommerce_tab_public_key'), $error);
						exit();
					}
				}else
				{
					echo "<span style='color:red'><h3>".__( 'Error, Please Try Again! The Captcha is Wrong!' , PW_FAQ_EXTRA_WOO_TABS_TEXTDOMAN )."</h3></span>";
					//$error = $resp->error;
					//echo recaptcha_get_html(get_option('woocommerce_tab_public_key'), $error);
					exit();
				}
			}else //if active captcha Not checked
			{
				$_product = $product->get_product($_POST['id_product']);
				$product_url = get_permalink( $_POST['id_product'] );
				$product_title= get_the_title( $_POST['id_product'] );
