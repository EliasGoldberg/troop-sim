	...
	<div id="<?php echo $perfix_tab;?>_tab" class="panel woocommerce_options_panel">
        ...
        <div class="wt-admingeneral wt-advanced">
            ...
            <?php
				...
				if ($WC_List_Grid->woocommerce_version_check('2.2')) {
			?>
            
            <input type="hidden" class="wc-product-search" style="width: 50%;" id="style_with_products" name="<?php echo $public_perfix.'tab_posts';?>[]" data-placeholder="<?php _e( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-action="woocommerce_json_search_products_and_variations" data-multiple="true"
            data-selected="<?php
					/*$product_field_type_ids =$custom_tab_options[$public_perfix.'tab_posts'];
					$product_ids = array_filter( array_map( 'absint', (array) $product_field_type_ids ) );*/
					
					
		  		   		// $product_field_type_ids =isset($custom_tab_options[$public_perfix.'tab_posts'][0]);
                    
					//$product_ids = ! empty( $product_field_type_ids ) ? array_map( 'absint',  $product_field_type_ids ) : null;
					
					
						//$product_ids =explode(',',$product_field_type_ids);
					 $product_field_type_ids =$custom_tab_options[$public_perfix.'tab_posts'];
                    $product_ids = ! empty( $product_field_type_ids ) ? array_map( 'absint',  $product_field_type_ids ) : null;	
					...
					$json_ids    = array();
 					...
 					if ( is_array($product_ids )) {
						...
						foreach ( $product_ids as $product_id ) {
							...
							$json_ids[ $product_id ] =  $product->get_formatted_name();
						}
					}
           ...
           <select id="style_with_products" name="<?php echo $public_perfix.'tab_posts';?>[]" class="ajax_chosen_select_products" multiple="multiple" data-placeholder="<?php _e( 'Search for a product&hellip;', EXTRA_WOO_TABS_TEXTDOMAN ); ?>">
            ...
            <?php } ?>  
            <img class="help_tip" data-tip='<?php _e( 'Choose Product', EXTRA_WOO_TABS_TEXTDOMAN ) ?>' src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" height="16" width="16" />
