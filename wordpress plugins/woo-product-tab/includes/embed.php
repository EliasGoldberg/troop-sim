...
function eb_admin_add_scripts(){
	...
	global $post_type;
	if((isset($_GET['post_type']) && $_GET['post_type']=='extra_product_tab') || 'extra_product_tab' == $post_type || (isset($_GET['post_type']) && $_GET['post_type']=='product') || 'product' == $post_type)
	{
		add_filter( 'wp_default_editor', 'pw_force_default_editor' );
		function pw_force_default_editor() {
			//allowed: tinymce, html, test
			return 'tinymce';
		}
	}else{
		add_filter( 'wp_default_editor', 'pw_force_default_editors' );
		function pw_force_default_editors() {
			//allowed: tinymce, html, test
			return 'html';
		}
	}
