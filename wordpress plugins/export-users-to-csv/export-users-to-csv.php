 ...
 */
/*
Plugin Name: Export Users to CSV (for GOWRT ONLY!)
Plugin URI: http://wordpress.org/extend/plugins/export-users-to-csv/
Description: Edited to return a CSV Made to import fields into S2Member
Version: 1.0.0
Author: Ulrich Sossou (edited by Matt Ortiz ;-)
...
class PP_EU_Export_Users {
	...
	public function generate_csv() {
		if ( isset( $_POST['_wpnonce-pp-eu-export-users-users-page_export'] ) ) {
			...
			$data_keys = array(
				'ID', 'user_login', 'Password', 'display_name',
				'user_nicename', 'user_email', 'user_url'
				);
			$meta_keys = $wpdb->get_results( "SELECT distinct(meta_key) FROM $wpdb->usermeta WHERE" );
			//$meta_keys = wp_list_pluck( $meta_keys, 'meta_key' );
			$meta_keys = array('first_name', 'last_name', 'wp_lxhv_capabilities', '_it_exchange_customer_subscription_ids', 
					'_it_exchange_stripe_subscription_id_live_mode', 'First Payment','Last Payment' ,'Auto-EOT Date' ,'Custom Value', 'SubscrID', 'gateway');
			 // echo "<pre>";
			 // var_dump($meta_keys);
			 // echo "</pre>";
			...
			foreach ( $users as $user ) {
				...
				foreach ( $fields as $field ) {
					//this just checks to makes sure they have an active acount so we dont have to mess with it later!!!
				if (key($user->$fields[10]) != ""){
					//echo "<pre>";
					//echo key($user->$fields[13]);
					//print_r($fields);  //get the array placements
					//echo "</pre>";

					$value = isset( $user->{$field} ) ? $user->{$field} : '';
					if($field == "_it_exchange_customer_subscription_ids"){
						$value = key($value);
					}
					elseif ($field == "wp_lxhv_capabilities") {
						if(key($value) == "subscriber"){
							$value = "s2member_level1";
						}
						else{
							$value = key($value);
						}
					...
					elseif ($field == 'SubscrID'){
						//Paypal ID
						if (key($user->$fields[10]) != ""){
							$value = key($user->$fields[10]);
						}
						else {$value = "";}
					}
					elseif ($field == 'gateway'){
						if (substr( key($user->$fields[10]), 0, 4 ) == "sub_"){
							$value = "stripe";
						}
						elseif (substr( key($user->$fields[10]), 0, 2 ) == "I-") {
							$value = "paypal";
						}
						else {$value = " ";}
...
<div class="wrap">
	...
	<form method="post" action="" enctype="multipart/form-data">
		...
		<h3>Just exporting users so we can import them into S2</h3>
...
<?php
	...
	public function exclude_data() {
		$exclude = array( 'user_activation_key' );
